<template>
    <div class="movie-concent">
      <div class="toup">体育</div>
       <div class="lis" v-for="(item,index) in sports" :key="index">
        <div class="img">
          <img :src="sports[index].picInfo[0]?sports[index].picInfo[0].url:$store.state.imgUrl" alt="">
        </div>
        <div class="title">{{item.title}}</div>
        <div class="webside">来自:{{item.source}}</div>
        <div class="ptime">{{item.ptime}}</div>
      </div>
      <div class="boxview" v-show="sports.length<1">
