<template>
    <div class="movie-concent">
      <div class="toup">热门推荐</div>
       <div class="lis" v-for="(item,index) in dy" :key="index">
        <div class="img">
          <img :src="dy[index].picInfo[0].url" alt="">
        </div>
        <div class="title">{{item.title}}</div>
        <div class="webside">来自:{{item.source}}</div>
        <div class="ptime">{{item.ptime}}</div>
      </div>
      <div class="boxview" v-show="dy.length<1">
